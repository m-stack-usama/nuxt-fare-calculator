...
import pkg from "@googlemaps/js-api-loader"; // Import the entire module
const { Loader } = pkg; // Destructure Loader from the module

export const useGoogleMaps = () => {
  const config = useRuntimeConfig(); // Access runtime config
  const loader = new Loader({
    apiKey: config.public.googleMapsApiKey, // Use Google Maps API key from config
